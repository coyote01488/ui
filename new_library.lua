			...
			function TabFunctions:InsertConfigSection(Side)
				...
				configSection:Button({
					...
					Callback = function()
						...
						WindowFunctions:Notify({
							Title = "Notification",
							Description = string.format("Successfully deleted config %q.", configSelection.Value),
						})
		...
		["Keybind"] = {
			Save = function(Flag, data)
				return {
					type = "Keybind", 
					flag = Flag, 
					bind = (MacLib.Options[Flag] and ((typeof(MacLib.Options[Flag]:GetBind()) == "EnumItem" and MacLib.Options[Flag]:GetBind().Name) or nil) or nil)
				}
			end,
			Load = function(Flag, data)
