...
function MacLib:Window(Settings)
	...
	macLib.Name = tostring(HttpService:GenerateGUID(false));
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				configSection:Button({
					...
					Callback = function()
						...
						WindowFunctions:Notify({
							Title = "Notification",
							Description = string.format("Successfully saved config %q.", configSelection.Value),
						})
					end,
				})
		...
		["Toggle"] = {
			...
			Load = function(Flag, data)
				if MacLib.Options[Flag] then
					MacLib.Options[Flag]:UpdateState(data.state and true or false)

					if MacLib.Options[Flag].Settings and MacLib.Options[Flag].Settings.Callback then
						MacLib.Options[Flag].Settings.Callback(MacLib.Options[Flag].State and true or false)
					end
				end
			end
		},
		...
		["Keybind"] = {
			Save = function(Flag, data)
				return {
					type = "Keybind", 
					flag = Flag, 
					bind = (typeof(data:GetBind()) == "EnumItem" and data:GetBind().Name) or nil
				}
			end,
			Load = function(Flag, data)
